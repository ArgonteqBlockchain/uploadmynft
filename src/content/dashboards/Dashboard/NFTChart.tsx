...
class NFTChart extends Component<ChartProps, ChartState> {
  ...
  drawChart() {
    ...
    const chartWidth: number = height ?? 250;
    const chartHeight: number = width ?? 250;
    ...
    totalForeground.transition().duration(1050).call(arcTween, 6.29, totalArc);

    const cirForeground = inCirculationSVG.append('path').datum({ endAngle: 0 }).style('fill', '#fec62d');
    cirForeground.transition().duration(1050).call(arcTween, inCirculationArcData, inCirculationArc);

    const stakedForground = stakedSVG.append('path').datum({ endAngle: 0 }).style('fill', '#ea3943');
    stakedForground.transition().duration(1050).call(arcTween, stakedArcData, stakedArc);
